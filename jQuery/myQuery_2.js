...
//attr 속성을 다루는 메소드
// attr (속성이름), (속성이름, 값), (속성이름, 콜백함수) 3가지일때 함수 처리 
// 두번째 값 매겨변수로 받음

$.fn.attr = function(p_attr, p_arg) {
    // console.log(this);   // this가 가르키는 것을 모를 경우 conole에 출력해봄, mydom
    if(typeof(p_attr) == "string" && !p_arg) {   // 읽기
        return this[0].getAttribute(p_attr);
    }
    if(typeof(p_attr) == "string" && typeof(p_arg) == "string") {   // 전부 쓰기
        for(var i = 0; i < this.length; i++) {
            this[i].setAttribute(p_attr, p_arg);
        }
        return this; // 이것은 메소드 체이닝을 위한 것
    }
    if(typeof(p_attr) == "string" && typeof(p_arg) == "function") { // 상황에 맞춰 쓰기
        for(var i = 0; i < this.length; i++) {
            this[i].setAttribute(p_attr, p_arg(i, this[i].getAttribute(p_attr)));
        }
        return this; // 메소드 체이닝 .의 마술
    }
    // JSON 타입일때
...
// $.fn.attr = function(p_attr, p_cb) {
    
//     for(var i = 0; i < this.length; i++) {
//         // 현재 요소가 (this[i])가 가지고 있느값
//         // p_cb(i, this[i].getAttribute(p_attr));
